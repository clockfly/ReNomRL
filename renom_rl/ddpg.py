...
class OU_noise(object):
    ...
    def sample(self):
        self.x0 = self.x0 + self.theta * \
            (self.mu - self.x0) * self.dt + self.sigma * \
        ...
        for i in range(num_episodes):
            ...
            for j in range(num_steps):
                ...
                if len(self._buffer) > self.batch_size:
                    ...
                    with self._critic.train():
                        ...
                        critic_loss = self.loss_func(value, target_q) + \
                            self.l2_decay * self._critic.weigiht_decay()
                    ...
                    with self._actor.train():
                        actor_loss = self.value_function(
                            train_prestate) + self.l2_decay * self._actor.weigiht_decay()
                ...
                if terminal:
                    ...
                    tq.set_description("episode: {:03d} Total reward:{:0.2f} avg loss:{:6.4f}".format(
                        i, ep_reward, float(loss) / (j + 1)))
                    tq.update(0)
                    tq.refresh()
                    tq.close()
                    break
