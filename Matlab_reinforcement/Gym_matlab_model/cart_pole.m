   ...
   %}
    ...
    methods
        function [next,reward,done]= forward(self,action,epi)
            ...
            if done==0
                reward=1;
            elseif self.steps_beyond_done ==0
        ...
        function CartPlot(self,acton,ep)
            if ep>500
                ...
                arrowfactor_x=sign(acton)*2.5;
                if (sign(arrowfactor_x)>0)
                    text_arrow = strcat('++>> ',num2str(10*acton));
                else
                    if (sign(arrowfactor_x)<0)
                        text_arrow = strcat(num2str(10*acton),' <<--');
                    else
                ...
                plot(pxp(2),pyp(2),'rO','LineWidth',2,'Markersize',10,'MarkerEdgeColor','k','MarkerFaceColor','r');
