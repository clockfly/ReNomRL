classdef dqn_with_target <handle 
    properties(SetAccess=private)
        a_w={rand( 4,20),rand(20,3)};% weight initialization
        t_w=[];% weight initialization for target which is copy 
        grads = {}; % for gradient
        learning_rate=0.001; 
        randstream
        seed224
   ...
   methods 
        ...
        function t_pre =t_predict(self,X_t) % for action prediction
            W1 = self.t_w(1);
            W2 = self.t_w(2);
            z_1=(X_t*W1); 
            H_1=tanh(z_1);
        ...
        function set_target_weight(self)
            a=self.a_w{1};
            b=self.a_w{1};
            self.t_w=[a,b];
        end
